# Lab 16: Resource Based Authorization - Distributed Application
...
# IMPORTANT: This lab assumes that you have a distributed application with a Blazor project, a Web API Project, a Duende Identity Server project and everything configured to use Oidc Auth. If you have a  monolithic application with a Blazor project with the Frontend, the Backend and the Identity provider all in one, skip this lab and follow lab 16 instead.
...
```bash
Add-Migration -Context PhotoSharingApplication.Infrastructure.Data.PhotosDbContext PhotoOwner -StartupProject PhotoSharingApplication -Project PhotoSharingApplication
Update-DataBase -Context PhotoSharingApplication.Infrastructure.Data.PhotosDbContext -StartupProject PhotoSharingApplication -Project PhotoSharingApplication
```
...
Create a new page `AccessDenied.razor` in the `Pages` folder to show an error message when the user is not authorized to access a page.
...
Both in the API and in the Blazor Client application, we should add an `Author` property to the `Comment` class.
...
```bash
Add-Migration CommentOwner -StartupProject PhotoSharingApplication.CommentsApi -Project PhotoSharingApplication.CommentsApi
Update-Database -StartupProject PhotoSharingApplication.CommentsApi -Project PhotoSharingApplication.CommentsApi
```
...
Register the policies in the `Program.cs`
...
Register the `CommentsService` and the `HttpContextAccessor` in the `Program.cs`
...
- The Blazor application should be updated to handle the `Unauthorized` response from the API to show the user an error message instead of crashing.

### The CommentsRepository

Both on the Blazor Server and the Blazor Client applications, we should update the `CommentsRepository` to handle the `Unauthorized` response from the API.
...
public async Task DeleteCommentAsync(int id)
{
    HttpResponseMessage response = await httpClient.DeleteAsync($"/apiauth/comments/{id}");
    if (response.StatusCode == System.Net.HttpStatusCode.Unauthorized)
    {
        throw new Exception("Unauthorized");
    }
...
Add a new property to store the error message and a boolean to control the visibility of the error message.
Try to add, delete or update a photo and eventually show the exception message in the error message.
